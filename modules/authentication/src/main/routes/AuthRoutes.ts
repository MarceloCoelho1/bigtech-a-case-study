...
export const authRouter = (app: FastifyInstance): void => {
  ...
  const authUseCases = new AuthUseCases(userRepository, jwtService, bcryptService, mailService);
  ...
  app.post('/auth/resetpassword/:token', adaptAuthRoute(authController, 'resetPassword'))
} 